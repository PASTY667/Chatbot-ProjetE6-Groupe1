...
class TestOllamaEmbeddingFunctionLive(unittest.TestCase):
    ...
    def test_live_call(self):
        ...
        valid = all(
            isinstance(emb, list)
            and len(emb) > 100
            and not any(x != x for x in emb)
            and sum(abs(x) for x in emb) > 0.1
    ...
    def test_live_embed_from_pdf(self):
        ...
        texts = [c["text"] for c in chunks[:6]]
    ...
    def test_batching_order_and_size(self):
        ...
        self.assertEqual(len(embeddings), len(texts))
        self.assertListEqual(calls, texts)

    def test_negative_batch_size_forced_to_one(self):
        ef = OllamaEmbeddingFunction()
        ef.EMBED_BATCH_SIZE = -5
        texts = ["t1", "t2", "t3"]
        ...
        def fake_post(self, url, json):
            calls.append(json["prompt"])
            return DummyResponse({"embedding": [1.0]})
